...
| extensive mass/tracer budget update | `src/fortran/conservative_transport.f90` | coordinate-independent finite-volume budget update from face-integrated mass fluxes, bounded face tracer fractions, explicit sources, and boundary/source diagnostics | flux reconstruction, geometry/velocity-to-flux conversion, CFL policy, high-order accuracy, monotonicity, or a complete atmospheric transport scheme |
...
| pressure-coordinate hydrostatics | `src/fortran/pressure_coordinate_hydrostatics.f90` | strictly ordered pressure interfaces mapped to layer mass, hypsometric geopotential/geometric thickness, and relative interface geopotential using declared layer-mean virtual temperature | pressure-coordinate continuity, hybrid/sigma coordinates, prognostic vertical motion, or a dynamical core |
| pressure-coordinate continuity | `src/fortran/pressure_coordinate_continuity.f90` | fixed-pressure-coordinate integration of interface pressure velocity `omega` from layer pressure-mean horizontal divergence and an explicit lower-boundary `omega`, plus local/column residual diagnostics | horizontal-divergence discretization, surface-pressure evolution, boundary closure policy, hybrid/sigma coordinates, geometric vertical velocity, momentum dynamics, or a complete dynamical core |
