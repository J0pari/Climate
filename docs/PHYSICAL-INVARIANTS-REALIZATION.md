...
`src/physical_budget_adapters.rs` is the narrow semantic bridge from physical-kernel diagnostics to that ledger. Conservative transport exposes lower/upper boundary contributions and separates nonnegative resolved sources from nonpositive resolved sinks for carrier and tracer mass; the adapter records those terms without reconstructing them from state deltas. Pressure-coordinate energy diagnostics are converted from specific power to extensive energy with explicit mass and timestep: kinetic pressure-gradient work, enthalpy pressure work, and the nonlocal part of geopotential material tendency are internal exchanges, while an explicit local geopotential tendency remains a separate coupling term. Any mismatch in the kernel closure survives as the ledger's unexplained residual rather than being relabeled.

Vertical diffusion exposes an instantaneous finite-volume budget rate with two separate mathematical statements. The thickness-weighted scalar inventory changes only through the signed lower and upper boundary fluxes. The derivative of one-half the thickness-weighted scalar quadratic splits exactly into nonpositive interior diffusion plus boundary quadratic exchange. The Rust adapter can record caller-integrated versions of both budgets, using a dedicated `resolved_dissipation` class for the quadratic loss. That quadratic quantity remains a declared scalar diagnostic; it is not physical energy unless the caller supplies an independently justified physical weighting and units.
