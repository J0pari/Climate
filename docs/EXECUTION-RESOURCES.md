# Execution resources and evidence boundaries
...
The repository supplies a dev-container bootstrap, a campaign runner, and a repository-side finite-resource ledger. Codespaces is disabled by default. Climate does not need the provider's account-wide billing total in order to limit its own use: every Climate-initiated campaign must have a committed authorization in `architecture/finite_resource_ledger.json` before a Codespace is provisioned or allowed to pass bootstrap. That authorization declares billable cores, a maximum wall time, reserved core-minutes, and reserved storage GiB-hours. The full reservation counts against Climate's self-budget before execution and is not automatically refunded because a run finished early.

The bootstrap claims the authorization before installing dependencies. The campaign reuses the same authorization, enforces the remaining wall-time on subprocess execution, and records a resource receipt with Codespace identity, detected CPUs, claim/finish times, actual wall seconds, rounded actual core-minutes, reserved/charged core-minutes and storage, and filesystem usage at claim/finish. Only one unreconciled authorization may exist. Before another campaign is allowed, the durable ledger must be updated from the prior receipt; completed entries charge the full reservation. This provides a conservative self-use ledger even if GitHub account-level usage APIs or billing pages are unavailable.

Provider billing data can be attached later as reconciliation evidence and remains useful for detecting usage outside Climate or differences in provider accounting, but it is not the mechanism that prevents Climate from overspending its own allocation. Account spending controls should additionally prevent unintended paid execution. Stop compute when a campaign is complete and delete unneeded environments because stopped Codespaces may continue to consume storage. Repetition merely to consume quota is not an experiment; repeated runs require a declared stochastic, robustness, scaling, or replication purpose.
