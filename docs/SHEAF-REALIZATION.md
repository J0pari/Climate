...
`data/ncei_ghcnd_bulk.py` is the first federation-scale observation provider adapter. `data/ncei_ghcnh_bulk.py` adds the second provider catalog: it parses NCEI's documented GHCNh global station list as a distinct namespace, uses exact shared GHCN identifiers as explicit digest-bound crosswalk evidence for stations common to GHCN-Daily and GHCNh, and creates separate roots for unmatched hourly stations. GHCNh hourly observations now enter through a raw preservation layer: annual captured tar archives are streamed station-member by station-member, validated against the federated alias/year identity, and published as immutable content-addressed spatial/year partitions whose source revision is the compressed archive SHA-256. Every PSV provider field is retained unchanged at this boundary; unit conversion, code interpretation, and quality-policy filtering remain explicit downstream transformations.

`data/ncei_ghcnd_bulk.py` is the first federation-scale provider adapter. It parses the complete GHCN-Daily station and inventory artifacts, preserves their content digests, normalizes station capability metadata into the federation identity layer, and creates bounded adaptive catalog shards. Observation acquisition uses the provider's bulk by-year artifacts rather than a loop of station-specific REST calls. The exact curl executable owns HTTP/TLS/redirect/transfer behavior; Climate supplies the explicit HTTPS URL and accepts only a successfully completed whole-file capture after recording the resolved curl binary path/version/SHA-256, byte count, and final artifact SHA-256. There is no Climate-owned HEAD/range/restart state machine: an interrupted transfer publishes no accepted artifact and a later attempt is a new execution. The REST adapter under `reference/` remains useful for captured subset/provenance witnesses; it is not the worldwide ingestion architecture. The by-year path is streamed row-by-row after capture: it validates the provider year/date contract, preserves missing values and measurement/quality/source flags, resolves each provider station through the captured federation catalog, and routes directly to caller-owned spatial/year/element partition sinks. No year-sized row list or station-by-time tensor is an ingestion prerequisite.
