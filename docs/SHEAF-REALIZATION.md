...
`data/ncei_ghcnd_bulk.py` is the first federation-scale provider parser/router. It parses complete GHCN-Daily station and inventory artifacts, preserves their content digests, normalizes station capability metadata into the federation identity layer, creates bounded adaptive catalog shards, and streams captured by-year rows into caller-owned spatial/year/element sinks. It contains no HTTP client, curl subprocess wrapper, redirect/range/retry logic, or transport checkpoint state. The execution environment invokes its exact native transfer tool directly, records that execution identity, and passes only completed immutable provider bytes into Climate. The REST reference likewise parses captured payloads rather than owning retrieval. No year-sized row list or station-by-time tensor is an ingestion prerequisite.
