...
`data/ncei_ghcnd_bulk.py` is the first federation-scale provider adapter. It parses the complete GHCN-Daily station and inventory artifacts, preserves their content digests, normalizes station capability metadata into the federation identity layer, and creates bounded adaptive catalog shards. Observation acquisition is addressed through the provider's bulk by-year artifacts rather than a loop of station-specific REST calls. The REST adapter under `reference/` remains useful for captured subset/provenance witnesses; it is not the worldwide ingestion architecture. The by-year path is streamed row-by-row: it validates the provider year/date contract, preserves missing values and measurement/quality/source flags, resolves each provider station through the captured federation catalog, and routes directly to caller-owned spatial/year/element partition sinks. No year-sized row list or station-by-time tensor is an ingestion prerequisite; durable Parquet/Zarr/object-store publication remains a separate storage boundary.
