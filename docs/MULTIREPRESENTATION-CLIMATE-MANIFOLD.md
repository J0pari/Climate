...
The first fitted rung uses ordinary controlled linear least squares with identical forcing-start and forcing-rate controls for two representations. The full two-layer temperature state is Markov-closed and should recover the exact fixed-step controlled transition from discovery-domain samples; surface temperature alone omits the deep-reservoir state and is expected to retain irreducible error on both discovery and held-out confirmation transitions. This is a deliberate case where a simpler representation loses for a structural reason rather than because the optimizer was weaker.
