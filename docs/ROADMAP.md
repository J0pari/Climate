...
| `multirepresentation.structure_baselines` — Test multirepresentation structure against simpler baselines | `ready` | `P1` | `R2_toolchain_ci` | — |
...
| `physics.parameterization_boundaries` — Establish physical parameterization seams and library boundaries | `blocked` | `P1` | `R2_toolchain_ci` | `physics.authoritative_flux_recomposition` |
| `simulation.reproducible_restart` — Define reproducible integration and restart state | `blocked` | `P1` | `R2_toolchain_ci` | `physics.authoritative_flux_recomposition`, `state.typed_physical_state` |
| `state.typed_physical_state` — Define typed physical-state boundaries | `blocked` | `P1` | `R2_toolchain_ci` | `physics.authoritative_flux_recomposition` |
| `adapters.versioned_array_ffi` — Define narrow versioned array FFI contracts | `blocked` | `P2` | `R2_toolchain_ci` | `state.typed_physical_state` |
