use std::collections::BTreeMap;

use climate_geometric_framework::sheaf::{EdgeRestriction, RealCellularSheaf, VertexStalk};
use nalgebra::DMatrix;
use serde::Deserialize;
use serde_json::json;

#[derive(Debug, Deserialize)]
struct Fixture {
    cases: Vec<Case>,
}

#[derive(Debug, Deserialize)]
struct Case {
    case_id: String,
    vertices: Vec<Vertex>,
    edges: Vec<Edge>,
    section: Vec<f64>,
}

#[derive(Debug, Deserialize)]
struct Vertex {
    id: String,
    dimension: usize,
}

#[derive(Debug, Deserialize)]
struct Edge {
    id: String,
    tail: String,
    head: String,
    edge_dimension: usize,
    tail_to_edge: Vec<Vec<f64>>,
    head_to_edge: Vec<Vec<f64>>,
}

fn fixture() -> Fixture {
    serde_json::from_str(include_str!(
        "../fixtures/sheaf/synthetic-linear-v1.json"
    ))
    .expect("synthetic sheaf fixture must parse")
}

fn dense(rows: &[Vec<f64>]) -> DMatrix<f64> {
    let cols = rows[0].len();
    let values: Vec<f64> = rows.iter().flat_map(|row| row.iter().copied()).collect();
    DMatrix::from_row_slice(rows.len(), cols, &values)
}

fn build_sheaf(case: &Case) -> RealCellularSheaf {
    let vertices = case
        .vertices
        .iter()
        .map(|vertex| VertexStalk::new(&vertex.id, vertex.dimension))
        .collect();
    let edges = case
        .edges
        .iter()
        .map(|edge| {
            EdgeRestriction::new(
                &edge.id,
                &edge.tail,
                &edge.head,
                edge.edge_dimension,
                dense(&edge.tail_to_edge),
                dense(&edge.head_to_edge),
            )
        })
        .collect();
    RealCellularSheaf::new(vertices, edges)
        .unwrap_or_else(|error| panic!("{}: invalid fixture sheaf: {error}", case.case_id))
}

fn vertex_offsets(case: &Case) -> BTreeMap<&str, (usize, usize)> {
    let mut offsets = BTreeMap::new();
    let mut offset = 0usize;
    for vertex in &case.vertices {
        offsets.insert(vertex.id.as_str(), (offset, vertex.dimension));
        offset += vertex.dimension;
    }
    assert_eq!(offset, case.section.len(), "{}: malformed section", case.case_id);
    offsets
}

fn apply(matrix: &[Vec<f64>], vector: &[f64]) -> Vec<f64> {
    matrix
        .iter()
        .map(|row| row.iter().zip(vector).map(|(a, b)| a * b).sum())
        .collect()
}

fn independent_local_residual_energy(case: &Case) -> f64 {
    let offsets = vertex_offsets(case);
    let mut energy = 0.0;
    for edge in &case.edges {
        let (tail_offset, tail_dimension) = offsets[edge.tail.as_str()];
        let (head_offset, head_dimension) = offsets[edge.head.as_str()];
        let tail = &case.section[tail_offset..tail_offset + tail_dimension];
        let head = &case.section[head_offset..head_offset + head_dimension];
        let tail_image = apply(&edge.tail_to_edge, tail);
        let head_image = apply(&edge.head_to_edge, head);
        for (tail_value, head_value) in tail_image.iter().zip(&head_image) {
            let residual = head_value - tail_value;
            energy += residual * residual;
        }
    }
    energy
}

fn matrix_max_abs_difference(left: &DMatrix<f64>, right: &DMatrix<f64>) -> f64 {
    assert_eq!(left.shape(), right.shape());
    let mut maximum = 0.0_f64;
    for row in 0..left.nrows() {
        for col in 0..left.ncols() {
            maximum = maximum.max((left[(row, col)] - right[(row, col)]).abs());
        }
    }
    maximum
}

#[test]
fn emit_preregistered_sheaf_claim_metrics() {
    let fixture = fixture();
    assert!(!fixture.cases.is_empty(), "fixture must be non-vacuous");

    let mut local_energy_max_abs_difference = 0.0_f64;
    let mut orientation_laplacian_max_abs_difference = 0.0_f64;

    for case in &fixture.cases {
        let sheaf = build_sheaf(case);
        let canonical = sheaf.compatibility_energy(&case.section).unwrap();
        let baseline = independent_local_residual_energy(case);
        local_energy_max_abs_difference =
            local_energy_max_abs_difference.max((canonical - baseline).abs());

        let reversed_edges = case
            .edges
            .iter()
            .map(|edge| {
                EdgeRestriction::new(
                    format!("{}_reversed", edge.id),
                    &edge.head,
                    &edge.tail,
                    edge.edge_dimension,
                    dense(&edge.head_to_edge),
                    dense(&edge.tail_to_edge),
                )
            })
            .collect();
        let vertices = case
            .vertices
            .iter()
            .map(|vertex| VertexStalk::new(&vertex.id, vertex.dimension))
            .collect();
        let reversed = RealCellularSheaf::new(vertices, reversed_edges).unwrap();
        orientation_laplacian_max_abs_difference = orientation_laplacian_max_abs_difference
            .max(matrix_max_abs_difference(&sheaf.laplacian_0(), &reversed.laplacian_0()));
    }

    let identity_case = fixture
        .cases
        .iter()
        .find(|case| case.case_id == "identity_path_clean")
        .expect("identity path case must exist");
    let identity_sheaf = build_sheaf(identity_case);
    let expected_graph_laplacian = DMatrix::from_row_slice(
        3,
        3,
        &[1.0, -1.0, 0.0, -1.0, 2.0, -1.0, 0.0, -1.0, 1.0],
    );
    let identity_graph_laplacian_max_abs_difference = matrix_max_abs_difference(
        &identity_sheaf.laplacian_0(),
        &expected_graph_laplacian,
    );

    let kernel_case = fixture
        .cases
        .iter()
        .find(|case| case.case_id == "nonconstant_projection_fault")
        .expect("kernel-translation case must exist");
    let kernel_sheaf = build_sheaf(kernel_case);
    let original = kernel_sheaf
        .compatibility_residual(&kernel_case.section)
        .unwrap();
    let mut translated = kernel_case.section.clone();
    translated[1] += 17.0;
    let shifted = kernel_sheaf.compatibility_residual(&translated).unwrap();
    let kernel_translation_residual_max_abs_difference = original
        .iter()
        .zip(shifted.iter())
        .map(|(left, right)| (left - right).abs())
        .fold(0.0_f64, f64::max);

    for value in [
        local_energy_max_abs_difference,
        identity_graph_laplacian_max_abs_difference,
        orientation_laplacian_max_abs_difference,
        kernel_translation_residual_max_abs_difference,
    ] {
        assert!(value <= 1e-12, "preregistered target missed: {value}");
    }

    let metrics = json!({
        "case_count": fixture.cases.len(),
        "sheaf.local_energy.max_abs_difference": local_energy_max_abs_difference,
        "sheaf.identity_graph.laplacian_max_abs_difference": identity_graph_laplacian_max_abs_difference,
        "sheaf.orientation.laplacian_max_abs_difference": orientation_laplacian_max_abs_difference,
        "sheaf.kernel_translation.residual_max_abs_difference": kernel_translation_residual_max_abs_difference
    });
    println!("CLIMATE_SHEAF_CLAIM_METRICS={metrics}");
}
